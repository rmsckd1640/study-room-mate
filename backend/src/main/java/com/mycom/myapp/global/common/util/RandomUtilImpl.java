...
public class RandomUtilImpl implements RandomUtils {
	...
	public Member randomMember() {
		return Member.builder().email(randomEmail()).password(randomString(8)).role(MemberRole.USER).name(randomString(5)).username(randomString(7)).build();
	}

	public Room randomRoom() {
		return Room.builder().capacity(randomInt(10)).name(randomString(3)).price(randomInt(3000)).build();
	}
